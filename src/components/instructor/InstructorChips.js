...
class InstructorChips extends React.Component {
  ...
  render() {
    if (!this.state.loadedAPI) {
      ...
      const y_tooltip =
        'According to our limited dataset, ' +
        data['name'] +
        ' has taught at OU since ' +
        data['most_recent_semester'] +
        '.';
      ...
      for (var i = 0; i < depts.length; i++) {
        ...
        depts_chips.push(
          <Tooltip
            ...
            enterTouchDelay={0}
            title={
              data['name'] + ' has taught in the ' + d + ' department/subject.'
            }
            classes={{
      ...
      return (
        <div>
          ...
          <div className="chip-container">
            <div className={classes.root}>{depts_chips}</div>
          </div>
        </div>
      );
    }
