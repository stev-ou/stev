...
class Fig2 extends React.Component {
	...
	render() {
		...
		var options = {
  		...
  		var data={
		labels:[result['dept']['dept name']+' Department Average', result['dept']['dept name']+''+ result['course number']+ ' Course Average'],
        datasets:[
          { label:'',
            data:[result['dept']['dept mean'].toFixed(2), result['current course mean'].toFixed(2)],
        ...
        for (var i=0; i<num_iterations; i++) {
        	...
        	data.datasets[0].data.push(result['instructors'][i]['instructor mean in course'].toFixed(2))
