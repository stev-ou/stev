...
class Fig3 extends React.Component {
  ...
  render() {
    if (!this.state.loadedAPI) {
      ...
      // Define the domain
      var domain = [Math.floor(Math.min(all_ratings)), Math.ceil(Math.max(all_ratings))]
      ...
      const myTimeSeries = TimeSeriesChart({data:data, cname:result['course over time']['course name'], dname:result['dept over time']['dept name'], colors:colors, domain:domain, lines:Lines});
...
const TimeSeriesChart = props => (
  ...
  <ResponsiveContainer width="90%" height={600} style={{display:'inline-block', align:'center'}}>
    <LineChart
      // width={800}
      // height={400}
      data={props.data}
    ...
    >
      ...
      <XAxis verticalAlign="bottom" dataKey="sem" padding={{ left: 30, right: 30 }}>
      <Label position='bottom' style={{ textAnchor: 'middle', padding:'0em', fontSize: 20}}>
        {"Semester of Interest"}
          </Label>
      </XAxis>
      <YAxis domain={props.domain} >
        <Label angle={270} position='left' style={{ textAnchor: 'middle', fontSize: 20}}>
        {"Rating (1 to 5, " + props.domain[0].toString() + '-' +props.domain[1].toString() + ' shown)'}
          </Label>
      </YAxis>
